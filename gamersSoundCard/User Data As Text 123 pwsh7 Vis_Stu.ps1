...
function InstallOffice365 {
    param (
        [string]$logFilePath,
        [string]$repoExtractPath
    )

    # Locate the OfficeSetup.exe file in the root of the repository
    $officeLocalInstallerPath = Join-Path $repoExtractPath "OfficeSetup.exe"

    if (!(Test-Path $officeLocalInstallerPath)) {
        Write-Host "OfficeSetup.exe not found in the repository root. Please make sure it is present."
        Add-Content -Path $logFilePath -Value "OfficeSetup.exe not found in the repository root."
        return
    }

    # Run the Office installer
    Add-Content -Path $logFilePath -Value "Installing Office 365"
    $process = Start-Process -FilePath $officeLocalInstallerPath -Wait -PassThru
    if ($process.ExitCode -eq 0) {
        Add-Content -Path $logFilePath -Value "Office 365 installation completed"
    } else {
        Add-Content -Path $logFilePath -Value "Office 365 installation failed with exit code: $($process.ExitCode)"
        Write-Host "Office 365 installation failed with exit code: $($process.ExitCode)"
    }
...
try {
    ...
    # Install Office 365
    try {
        InstallOffice365 -logFilePath $logFilePath -repoExtractPath $repoExtractPath
    } catch {
